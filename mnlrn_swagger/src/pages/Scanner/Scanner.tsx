...
export const Scanner = () => {
    ...
    const [filteredTransactionsList, setFilteredTransactionsList] = useState<any[]>([]);
    ...
    const getTransactionsList = async (address: string) => {
        ...
        const list = await getApiFullGeneric(`transactions?sender=erd1n3fzpwd9m6z35s9lqksxqnhmkyxatu20qp2lvtrdhdx6f4s8ypjsqskr84`, { pageSize: 1000 });
    ...
    useEffect(() => {
        getTransactionsList("erd1n3fzpwd9m6z35s9lqksxqnhmkyxatu20qp2lvtrdhdx6f4s8ypjsqskr84");
    }, []);
    ...
    return (
        <div className='flex flex-col gap-6 max-w-7xl w-full'>
            <Card
                ...
                description={'Latest interactions of a wallet with different smart contracts'}
                reference={''}
            >
                {transactionsList.map((transaction, index) => (
                    <div key={index}>
                        {transaction.receiver} | {transaction.function} | {transaction.timestamp}
                    </div>
                ))}
                <br></br>
                <br></br>
