...
export const Dashboard = () => {
  ...
  const loadProjects = async () => {
    ...
    for (let i = 0; i < endpointsIds.length; i++) {
      const endpoint = await turbodefiContractService.getEndpointById(endpointsIds[i]);
      if (endpoint.project) {
        const structReader = (structReaders[endpoint.project]) ? structReaders[endpoint.project] : await loadStructReader(endpoint.project);
                    ...
                    {/* TODO LOAD HERE ALL USER PANELS IN DASHBOARD */}
                    {
                      endpoints.map((endpoint, index) => (
                        <div className='w-full border rounded-lg' key={index}>
                          <ProjectEndpointForm
                            module={endpoint.structReader.getModule(endpoint.endpoint.module || '')}
